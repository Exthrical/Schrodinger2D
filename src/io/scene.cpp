...
static double extract_number(const std::string& s, const std::string& key, double def) {
    std::regex re(key + R"_("\s*:\s*([-+eE0-9\.]+))_");
    std::smatch m; if (std::regex_search(s, m, re)) return std::stod(m[1].str());
    return def;
}

static int extract_int(const std::string& s, const std::string& key, int def) {
    std::regex re(key + R"_("\s*:\s*([-+0-9]+))_");
...
bool load_scene(const std::string& path, Scene& sc) {
    ...
    // Very simple array parsing for boxes and packets created by our writer
    sc.boxes.clear();
    sc.packets.clear();
...
void to_simulation(const Scene& s, sim::Simulation& sim) {
    ...
    sim.pfield.cap_ratio = s.cap_ratio; sim.pfield.cap_strength = s.cap_strength;
